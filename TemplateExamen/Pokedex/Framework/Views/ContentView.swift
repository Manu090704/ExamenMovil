...
struct ContentView: View {
    ...
    @State private var viewMode: CovidViewMode = .single
        ...
        NavigationView {
            VStack {
                ...
                Picker("Modo", selection: $viewMode) {
                    Text("📈 Vista individual").tag(CovidViewMode.single)
                    Text("🔀 Comparativa").tag(CovidViewMode.compare)
                }
                ...
                if viewMode == .single {
                    ...
                    CovidCompareView() // 👈 tu vista comparativa
                }
            }
    ...
    /// VISTA INDIVIDUAL
    /// ===================
    var singleCountryView: some View {
        ScrollView {
            VStack(spacing: 24) {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    Picker("País", selection: $vm.selectedCountry) {
                        ForEach(countries, id: \.self) { c in
                            Text(c)
                        }
                    }
                    .pickerStyle(.menu)
                    .onChange(of: vm.selectedCountry) { new in
                ...
                if let err = vm.errorMessage {
                    ...
                    Button("Reintentar") {
                        Task { await vm.fetch(country: vm.selectedCountry) }
                    }
        ...
        .task {
            await vm.loadLastSelection()
        }
        .task {
            await vm.fetch(country: vm.selectedCountry)
        }
    }
}
